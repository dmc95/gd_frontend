...
const Login = ({ setUser }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("https://gd-backend-ggq9.onrender.com/users/login", {
        email: email,
        password: password,
      });
  ...
  return (
    <>
      ...
      <div className="pt-[200px] flex flex-col justify-center items-center w-screen h-screen">
        ...
        >
          <input
            className="border py-3 rounded-xl w-5/6 p-1 text-black"
            placeholder="email"
            type="email"
            value={email}
            onChange={(event) => {
          ...
          <input
            ...
            className="cursor-pointer border bg-lime-200 hover:bg-lime-100 font-semibold py-3 rounded-xl w-5/6 border border-green-400"
            type="submit"
          />
        </form>
      </div>
