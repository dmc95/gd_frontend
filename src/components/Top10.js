...
const Top10 = ({ setSelectedSymbol }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://gd-backend-ggq9.onrender.com/daily_trade_top"
        );
  ...
  return isLoading ? (
    ...
    <div className="h-[550px] 2xl:h-[500px] 3xl:h-[750px] overflow-y-scroll mt-20 z-20">
      {data.map((elem, key) => (
        <div
          onClick={() => handleClick(elem.symbol)}
          key={key}
          className="cursor-pointer flex flex-row justify-evenly 2xl:my-8 my-3 p-2 text-lg border borderColor rounded-lg hover:translate-y-1 shadow-md backdrop-blur-sm bg-white/30"
        >
