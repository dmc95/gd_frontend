...
const News = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://finnhub.io/api/v1/company-news?symbol=AAPL&from=${dateHierFormatee}&to=${dateFormatee}&token=${process.env.REACT_APP_FINNHUB_TOKEN}`
        );
  ...
  return isLoading ? (
    ...
    <>
      ...
      <div className="h-[750px] rounded-lg mx-2 z-0 rounded-lg border borderColor bg-black/10 backdrop-blur-[1px] p-3 pb-10">
            ...
            elem.image ? (
              <a href={elem.url} rel="noreferrer" target="_blank">
                <div key={index} className="w-[240px] flex-shrink-0 mx-5 animate-scroll ">
                  <img
                    className="w-full h-[200px] object-contain hover:scale-102 translate-x-1 duration-100 "
                    src={elem.image}
                    alt=""
                  />
