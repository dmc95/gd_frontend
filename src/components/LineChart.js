...
const LineChart = ({ selectedSymbols, filter }) => {
      ...
      console.log("Filter:", filter);
        ...
        const promises = selectedSymbols.map(async (symbol) => {
          const response = await axios.get(
            `https://gd-backend-ggq9.onrender.com/daily_trade_periode/${symbol}&${filter}`
          );
          return {
            symbol: symbol,
            data: response.data,
  ...
  return isLoading ? (
   <p className="flex justify-center m-2">Cliquez sur un indice ETF pour afficher le graphique</p>
  ) : (
